...
public class CloneTests {
    ...
    public void setUp() throws Exception {
        ...
        for (Player p : game.getAllPlayers())
            p.setDecider(new DominionDeciderContainer(defaultPurchaseDecider, hardCodedActionDecider));
    }
    ...
    @Test
    public void cloneWithGardensGivesCorrectVP() {
        game.getPlayer(2).insertCardDirectlyIntoHand(CardFactory.instantiateCard(CardType.GARDENS));

        for (int i = 1; i <= 4; i++) {
            game.getPlayer(i).refreshPositionSummary();
            if (i == 2) {
                assertEquals(game.getPlayer(i).totalVictoryValue(), 4);
                assertEquals(game.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), 1, 0.01);
            } else {
                assertEquals(game.getPlayer(i).totalVictoryValue(), 3);
                assertEquals(game.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), -1, 0.01);
            }
        }
        DominionGame clonedGame = game.clone(game.getCurrentPlayer());
        ...
        for (int i = 1; i <= 4; i++) {
            if (i == 2)
                assertEquals(clonedGame.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), 1, 0.01);
            else
                assertEquals(clonedGame.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), -1, 0.01);
        }
    }
