...
public class PositionSummaryTest {
    ...
    public void victoryMargin() {
        for (int i = 1; i <= 4; i++) {
            assertEquals(game.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), 0.0, 0.01);
        }
        Player p3 = game.getPlayer(3);
        p3.insertCardDirectlyIntoHand(CardFactory.instantiateCard(CardType.DUCHY));
        for (int i = 1; i <= 4; i++) {
            game.getPlayer(i).refreshPositionSummary();
            if (i == 3)
                assertEquals(game.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), 3.0, 0.01);
            else
                assertEquals(game.getPlayer(i).getPositionSummaryCopy().getVictoryMargin(), -3.0, 0.01);
        }
    }
