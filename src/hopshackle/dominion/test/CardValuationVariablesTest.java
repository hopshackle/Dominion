...
public class CardValuationVariablesTest {
    ...
    public void basicVariablesFromCardPlayPlayedI() {
        ...
        assertEquals(CardValuationVariables.WORKSHOP_IN_PLAY.getValue(p1), 0, 0.001);
    ...
    public void cardsInPlay() {
        Player player = game.getCurrentPlayer();
        player.setDecider(remodelDecider);
        player.insertCardDirectlyIntoHand(CardFactory.instantiateCard(CardType.REMODEL));
        player.insertCardDirectlyIntoHand(CardFactory.instantiateCard(CardType.WORKSHOP));
        player.setState(Player.State.PLAYING);
        p1 = player.getPositionSummaryCopy();
        assertEquals(CardValuationVariables.REMODEL_IN_PLAY.getValue(p1), 0, 0.001);
        assertEquals(CardValuationVariables.WORKSHOP_IN_PLAY.getValue(p1), 0, 0.001);
        p1 = p1.apply(CardTypeAugment.playCard(CardType.REMODEL));    // This is just a lookahead and does not update the game
        assertEquals(CardValuationVariables.REMODEL_IN_PLAY.getValue(p1), 1, 0.001);
        assertEquals(CardValuationVariables.WORKSHOP_IN_PLAY.getValue(p1), 0, 0.001);
